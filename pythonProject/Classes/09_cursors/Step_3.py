...
input_shp = r'C:\Peck_NRS528_PythonGIS\pythonProject\Classes\09_cursors\Step_3_Data\URI_Campus_Roads_OSM.shp'
...
# First let's see if our code works..
path_expression = arcpy.AddFieldDelimiters(input_shp, "fclass") + " = 'paths'"

print("Executing UpdateCursor using Expression: " + path_expression)

with arcpy.da.SearchCursor(input_shp, fields, path_expression) as cursor:
    for row in cursor:
        print(u'{0}, {1}, {2}'.format(row[0], row[1], row[2]))
